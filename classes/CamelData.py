...
class LazyCamelDataset:
    
    num_camel : int
    locations_per_camel : int
    
    def __init__(self, num_camel, locations_per_camel, std_background = 1):
        super().__init__()
        
        self.num_camel = num_camel
        self.locations_per_camel = locations_per_camel
        
        self.std_background = std_background

    
    def __call__(self):
        ys_truth, ys_corrupt,_ = generate_camels( num_camel = self.num_camel,
                                                locations_per_camel = self.locations_per_camel,
                                                std_background = self.std_background)
        
        return tc.Tensor(ys_truth), tc.Tensor(ys_corrupt)
